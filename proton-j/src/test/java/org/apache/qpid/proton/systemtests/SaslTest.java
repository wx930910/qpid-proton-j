...
public class SaslTest extends EngineTestBase {
	public static SaslListener mockSaslListener2(AtomicBoolean mechanismsReceived, AtomicBoolean challengeReceived,
			AtomicBoolean outcomeReceived) {
		AtomicBoolean[] mockFieldVariableMechanismsReceived = new AtomicBoolean[] { new AtomicBoolean() };
		AtomicBoolean[] mockFieldVariableChallengeReceived = new AtomicBoolean[] { new AtomicBoolean() };
		AtomicBoolean[] mockFieldVariableOutcomeReceived = new AtomicBoolean[] { new AtomicBoolean() };
		SaslListener mockInstance = Mockito.mock(SaslListener.class);
		mockFieldVariableMechanismsReceived[0] = mechanismsReceived;
		mockFieldVariableChallengeReceived[0] = challengeReceived;
		mockFieldVariableOutcomeReceived[0] = outcomeReceived;
		Mockito.doAnswer((stubInvo) -> {
			Sasl s = stubInvo.getArgument(0);
			...
			assertArrayEquals("Client should now know the server's outcome additional data",
					clientReceivedAdditionalBytes, clientReceivedAdditionalBytes);
			assertFalse("Should not have already received outcome",
					mockFieldVariableOutcomeReceived[0].getAndSet(true));
			return null;
		}).when(mockInstance).onSaslOutcome(Mockito.any(), Mockito.any());
		Mockito.doAnswer((stubInvo) -> {
			Sasl s = stubInvo.getArgument(0);
			assertArrayEquals("Client should now know the server's mechanisms.", new String[] { TESTMECH1, TESTMECH2 },
					s.getRemoteMechanisms());
			...
			assertFalse("Should not have already received mechanisms",
					mockFieldVariableMechanismsReceived[0].getAndSet(true));
			return null;
		}).when(mockInstance).onSaslMechanisms(Mockito.any(), Mockito.any());
		Mockito.doAnswer((stubInvo) -> {
			Sasl s = stubInvo.getArgument(0);
			...
			assertEquals("Unexpected SASL outcome at client", SaslOutcome.PN_SASL_NONE, s.getOutcome());
			assertArrayEquals("Client should now know the server's challenge", CHALLENGE_BYTES,
					clientReceivedChallengeBytes);
			s.send(RESPONSE_BYTES, 0, RESPONSE_BYTES.length);
			assertFalse("Should not have already received challenge",
					mockFieldVariableChallengeReceived[0].getAndSet(true));
			return null;
		}).when(mockInstance).onSaslChallenge(Mockito.any(), Mockito.any());
		return mockInstance;
	}

	public static SaslListener mockSaslListener1(AtomicBoolean initReceived, AtomicBoolean responseReceived) {
		AtomicBoolean[] mockFieldVariableInitReceived = new AtomicBoolean[] { new AtomicBoolean() };
		AtomicBoolean[] mockFieldVariableResponseReceived = new AtomicBoolean[] { new AtomicBoolean() };
		SaslListener mockInstance = Mockito.mock(SaslListener.class);
		mockFieldVariableInitReceived[0] = initReceived;
		mockFieldVariableResponseReceived[0] = responseReceived;
		Mockito.doAnswer((stubInvo) -> {
			Sasl s = stubInvo.getArgument(0);
			...
			assertArrayEquals("Server should now know the client's response", RESPONSE_BYTES,
					serverReceivedResponseBytes);
			s.send(ADDITIONAL_DATA_BYTES, 0, ADDITIONAL_DATA_BYTES.length);
			s.done(SaslOutcome.PN_SASL_OK);
			assertFalse("Should not have already received response",
					mockFieldVariableResponseReceived[0].getAndSet(true));
			return null;
		}).when(mockInstance).onSaslResponse(Mockito.any(), Mockito.any());
		Mockito.doAnswer((stubInvo) -> {
			Sasl s = stubInvo.getArgument(0);
			...
			assertArrayEquals("Server should now know the client's initial response.", INITIAL_RESPONSE_BYTES,
					serverReceivedInitialBytes);
			s.send(CHALLENGE_BYTES, 0, CHALLENGE_BYTES.length);
			assertFalse("Should not have already received init", mockFieldVariableInitReceived[0].getAndSet(true));
			return null;
		}).when(mockInstance).onSaslInit(Mockito.any(), Mockito.any());
		return mockInstance;
	}
	...
	public void testSaslNegotiation() throws Exception {
		...
		assertEquals("Server should not yet know the remote's chosen mechanism.", 0,
	...
	/**
	 * 5.3.2 SASL Negotiation. ...challenge/response step can occur zero or more
	 * times
	 */
	@Test
	...
	/**
	 * 5.3.3.5 The additional-data field carries additional data on successful
	 * authentication outcome as specified by the SASL specification [RFC4422].
	 */
	@Test
	public void testOutcomeAdditionalData() throws Exception {
		...
		assertEquals("Unexpected SASL outcome at client", SaslOutcome.PN_SASL_OK, clientSasl.getOutcome());
		assertArrayEquals("Client should now know the serrver's additional-data", ADDITIONAL_DATA_BYTES,
				clientReceivedAdditionalDataBytes);
	}

	/**
	 * 5.3.3.6 Connection authentication failed due to an unspecified problem with
	...
	/*
	 * Test that transports configured to do so are able to perform SASL process
	 * where frames are exchanged larger than the 512byte min-max-frame-size.
	 */
	@Test
	public void testSaslNegotiationWithConfiguredLargerFrameSize() throws Exception {
		...
		// Select a mech, send large initial response along with it in sasl-init, verify
		// server receives it
		clientSasl.setMechanisms(TESTMECH1);
		byte[] initialResponseBytes = Arrays.copyOf(largeInitialResponseBytesOrig,
				largeInitialResponseBytesOrig.length);
		...
		assertArrayEquals("Server should now know the client's response", largeResponseBytesOrig,
				serverReceivedResponseBytes);

		// Send an outcome with large additional data in a sasl-outcome, verify client
		// receives it
	...
	public void testSaslNegotiationUsingListener() throws Exception {
		...
		Sasl clientSasl = getClient().transport.sasl();
		clientSasl.client();
		clientSasl.setListener(SaslTest.mockSaslListener2(mechanismsReceived, challengeReceived, outcomeReceived));
		...
		serverSasl.server();
		serverSasl.setMechanisms(TESTMECH1, TESTMECH2);
		serverSasl.setListener(SaslTest.mockSaslListener1(initReceived, responseReceived));
