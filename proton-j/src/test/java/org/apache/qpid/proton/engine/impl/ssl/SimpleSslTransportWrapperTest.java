...
/**
 * TODO unit test handshaking TODO unit test closing TODO unit test graceful
 * handling of SSLEngine.wrap throwing an SSLException
 */
public class SimpleSslTransportWrapperTest {
	...
	/**
	 * Note that this only feeds 1 encoded packet in at a time due to default
	 * settings of the dummy engine, See
	 * {@link #testUnderlyingInputUsingSmallBuffer_receivesAllDecodedInputRequiringMultipleUnwraps}
	 * for a related test that passes multiple encoded packets into the ssl wrapper
	 * at once.
	 */
	@Test
	...
	/**
	 * As per
	 * {@link #testInputIncompletePacket_isNotPassedToUnderlyingInputUntilCompleted()}
	...
	public void testInputIncompletePacketInThreeParts() {
		...
		String secondEncodedBytes = "C"; // Sending this causes the impl to have to hold the data without producing more
											// input yet
	...
	/**
	 * Tests that when a small underlying input buffer (1 byte here) is used, all of
	 * the encoded data packet (5 bytes each here) can be processed despite multiple
	 * attempts being required to pass the decoded bytes (2 bytes here) to the
	 * underlying input layer for processing.
	 */
	@Test
	public void testUnderlyingInputUsingSmallBuffer_receivesAllDecodedInputRequiringMultipleUnderlyingProcesses() {
		...
		assertEquals("Unexpected max encoded chunk size", encodedPacketSize,
				CapitalisingDummySslEngine.MAX_ENCODED_CHUNK_SIZE);
		...
		assertEquals("Unexpected number of bytes poured into the wrapper input buffer", encodedPacketSize,
				numberPoured);
		assertEquals("Unexpected position in encoded source byte buffer", encodedPacketSize * 1,
				encodedByteSource.position());
		...
		assertEquals("unexpected underlying output after first wrapper process", "a_",
				_underlyingInput.getAcceptedInput());
		assertEquals("unexpected underlying process count after first wrapper process", 2,
				_underlyingInput.getProcessCount());

		// Process the second 'encoded packet' (<-B->)
		numberPoured = pour(encodedByteSource, _sslWrapper.tail());
		assertEquals("Unexpected number of bytes poured into the wrapper input buffer", encodedPacketSize,
				numberPoured);
		assertEquals("Unexpected position in encoded source byte buffer", encodedPacketSize * 2,
				encodedByteSource.position());
		assertEquals("Unexpected capacity", 0, _sslWrapper.capacity());
		_sslWrapper.process();
		assertEquals("Unexpected capacity", encodedPacketSize, _sslWrapper.capacity());

		assertEquals("unexpected underlying output after second wrapper process", "a_b_",
				_underlyingInput.getAcceptedInput());
		assertEquals("unexpected underlying process count after second wrapper process", 4,
				_underlyingInput.getProcessCount());
	}

	/**
	 * Tests that when a small underlying input buffer (1 byte here) is used, all of
	 * the encoded data packets (20 bytes total here) can be processed despite
	 * multiple unwraps being required to process a given set of input (3 packets,
	 * 15 bytes here) and then as a result also multiple attempts to pass the
	 * decoded packet (2 bytes here) to the underlying input layer for processing.
	 */
	@Test
	public void testUnderlyingInputUsingSmallBuffer_receivesAllDecodedInputRequiringMultipleUnwraps() {
		...
		// Create a dummy ssl engine that has buffers that holds multiple
		// encoded/decoded
		...
		// Process the first three 'encoded packets' (<-A-><-B-><-C->). This will
		// require 3 'proper' unwraps, and
		// as each decoded packet is 2 bytes, each of those will require 2 underlying
		// input processes.
		int numberPoured = pour(encodedByteSource, _sslWrapper.tail());
		assertEquals("Unexpected number of bytes poured into the wrapper input buffer", sslEngineBufferSize,
				numberPoured);
		assertEquals("Unexpected position in encoded source byte buffer", encodedPacketSize * 3,
				encodedByteSource.position());
		assertEquals("Unexpected capacity", 0, _sslWrapper.capacity());
		...
		assertEquals("unexpected underlying process count after wrapper process", 6,
				_underlyingInput.getProcessCount());
		...
		assertEquals("Unexpected number of bytes poured into the wrapper input buffer", encodedPacketSize,
				numberPoured);
		assertEquals("Unexpected position in encoded source byte buffer", encodedPacketSize * 4,
				encodedByteSource.position());
		...
		assertEquals("a_b_c_d_", _underlyingInput.getAcceptedInput());
		assertEquals("Unexpected capacity", sslEngineBufferSize, _sslWrapper.capacity());
		assertEquals("unexpected underlying process count after second wrapper process", 8,
				_underlyingInput.getProcessCount());
		assertEquals(6, _dummySslEngine.getUnwrapCount()); // earlier + 1 decode + 1 underflow
	}

	/**
	 * Tests that an exception is thrown when the underlying input has zero capacity
	 * when the call with newly decoded input is initially made.
	 */
	@Test(timeout = 5000)
	public void testUnderlyingInputHasZeroCapacityInitially() {
		...
		// Set the input to have a small buffer, but then return 0 from the 2nd capacity
		// call onward.
		_underlyingInput.setInputBufferSize(underlyingInputBufferSize);
		_underlyingInput.setZeroCapacityAtCount(2);
		assertEquals("Unexpected initial underlying input capacity", underlyingInputBufferSize,
				_underlyingInput.capacity());
		...
		assertEquals("Unexpected number of bytes poured into the wrapper input buffer", encodedPacketSize,
				numberPoured);
		assertEquals("Unexpected position in encoded source byte buffer", encodedPacketSize,
				encodedByteSource.position());
		...
		assertEquals("unexpected underlying process count after wrapper process", 0,
				_underlyingInput.getProcessCount());
		assertEquals("unexpected underlying capacity count after wrapper process", 3,
				_underlyingInput.getCapacityCount());
	...
	/**
	 * Tests that an exception is thrown when the underlying input has no capacity
	 * (but isn't closed) during the process of incrementally passing the decoded
	 * bytes to its smaller input buffer for processing.
	 */
	@Test(timeout = 5000)
	public void testUnderlyingInputHasZeroCapacityMidProcessing() {
		...
		// Set the input to have a small buffer, but then return 0 from the 3rd capacity
		// call onward.
		_underlyingInput.setInputBufferSize(underlyingInputBufferSize);
		...
		assertEquals("Unexpected wrapper capacity", -1, _sslWrapper.capacity());
		assertEquals("unexpected underlying process count after wrapper process", 1,
				_underlyingInput.getProcessCount());
		assertEquals("unexpected underlying capacity count after wrapper process", 3,
				_underlyingInput.getCapacityCount());
		assertEquals("unexpected underlying capacity after wrapper process", 0, _underlyingInput.capacity());
	...
	private void putBytesIntoTransport(String encodedBytes) {
		...
		while (byteBuffer.hasRemaining()) {
			...
			assertTrue("We should be able to pour some bytes into the input buffer", numberPoured > 0);
			_sslWrapper.process();
		}
	}
