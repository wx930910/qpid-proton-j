...
public class ReactorTest {
	...
	public static BaseHandler mockBaseHandler1(int count) {
		int[] mockFieldVariableRemaining = new int[1];
		BaseHandler mockInstance = Mockito.spy(BaseHandler.class);
		mockFieldVariableRemaining[0] = count;
		Mockito.doAnswer((stubInvo) -> {
			Event event = stubInvo.getArgument(0);
			Sender link = (Sender) event.getLink();
			while (link.getCredit() > 0 && mockFieldVariableRemaining[0] > 0) {
				Delivery dlv = link.delivery(new byte[0]);
				assertNotNull(dlv);
				dlv.settle();
				link.advance();
				--mockFieldVariableRemaining[0];
			}
			if (mockFieldVariableRemaining[0] == 0) {
				event.getConnection().close();
			}
			return null;
		}).when(mockInstance).onLinkFlow(Mockito.any());
		Mockito.doAnswer((stubInvo) -> {
			Event event = stubInvo.getArgument(0);
			event.getConnection().free();
			return null;
		}).when(mockInstance).onConnectionRemoteClose(Mockito.any());
		Mockito.doAnswer((stubInvo) -> {
			Event event = stubInvo.getArgument(0);
			...
			snd.open();
			return null;
		}).when(mockInstance).onConnectionInit(Mockito.any());
		return mockInstance;
	}
	...
	public static Collection<ReactorFactory[]> data() throws IOException {
		ReactorFactory classicReactor = new ReactorFactory() {
			@Override
			public Reactor newReactor() throws IOException {
	 ...
	 * <li>Returns immediately from the run method (as there is no more work to
	 * do).</li>
	 * </ul>
	 * 
	 * @throws IOException
	...
	/**
	 * Tests adding a handler to a reactor and running the reactor. The expected
	 * behaviour is for the reactor to return, and a number of reactor- related
	 * events to have been delivered to the handler.
	 * 
	 * @throws IOException
	 */
	@Test
	public void handlerRun() throws IOException {
		...
		reactor.run();
		reactor.free();
		testHandler.assertEvents(Type.REACTOR_INIT, Type.SELECTABLE_INIT, Type.SELECTABLE_UPDATED,
	...
	public void connection() throws IOException {
		...
		assertEquals("connection address configuration failed", reactor.getConnectionAddress(connection),
				"127.0.0.1:5672");
		TestHandler reactorHandler = new TestHandler();
		reactor.getHandler().add(reactorHandler);
		reactor.run();
		reactor.free();
		reactorHandler.assertEvents(Type.REACTOR_INIT, Type.SELECTABLE_INIT, Type.SELECTABLE_UPDATED,
				Type.SELECTABLE_FINAL, Type.REACTOR_FINAL);
		connectionHandler.assertEvents(Type.CONNECTION_INIT);
	}

	/**
	 * Tests operation of the Reactor.acceptor method by creating an acceptor which
	 * is immediately closed by the reactor. The expected behaviour is for:
	 * <ul>
	...
	public void acceptor() throws IOException {
		...
		assertFalse("acceptor should have been removed from the reactor's children",
	...
	/**
	 * Tests end to end behaviour of the reactor by creating an acceptor and then a
	 * connection (which connects to the port the acceptor is listening on). As soon
	 * as the connection is established, both the acceptor and connection are
	 * closed. The events generated by the acceptor and the connection are compared
	...
	public void connect() throws IOException {
		...
		class ClientHandler extends TestHandler {
			...
			public void onConnectionInit(Event event) {
				...
				event.getReactor().setConnectionHost(event.getConnection(), "127.0.0.1", listeningPort);
				event.getConnection().open();
			}
		...
		assertFalse("acceptor should have been removed from the reactor's children",
				reactor.children().contains(acceptor));
		assertFalse("connection should have been removed from the reactor's children",
		...
		sh.assertEvents(Type.CONNECTION_INIT, Type.CONNECTION_BOUND,
				...
				Type.CONNECTION_REMOTE_OPEN, Type.CONNECTION_LOCAL_OPEN, Type.TRANSPORT, Type.CONNECTION_REMOTE_CLOSE,
				Type.TRANSPORT_TAIL_CLOSED, Type.CONNECTION_LOCAL_CLOSE, Type.TRANSPORT, Type.TRANSPORT_HEAD_CLOSED,
				Type.TRANSPORT_CLOSED, Type.CONNECTION_UNBOUND, Type.CONNECTION_FINAL);
		...
		ch.assertEvents(Type.CONNECTION_INIT, Type.CONNECTION_LOCAL_OPEN, Type.CONNECTION_BOUND,
				...
				Type.CONNECTION_REMOTE_OPEN, Type.CONNECTION_LOCAL_CLOSE, Type.TRANSPORT, Type.TRANSPORT_HEAD_CLOSED,
				Type.CONNECTION_REMOTE_CLOSE, Type.TRANSPORT_TAIL_CLOSED, Type.TRANSPORT_CLOSED,
				Type.CONNECTION_UNBOUND, Type.CONNECTION_FINAL);
	...
	private String checkVhost(String vhost) throws IOException {
		...
		class ClientVhostHandler extends TestHandler {
			...
			public void onConnectionInit(Event event) {
				super.onConnectionInit(event);
				event.getReactor().setConnectionHost(event.getConnection(), "127.0.0.1", port);
				if (vhost != null) {
	...
	/**
	 * Tests the virtual host default configuration - should be set to host used for
	 * the connection.
	 * 
	 * @throws IOException
	 **/
	...
	public void checkVhostDefault() throws IOException {
		...
		assertEquals("The default virtual host is not correct", "127.0.0.1", vhost);
	}

	/**
	 * Tests the virtual host override - should be set to connection's hostname.
	 * 
	 * @throws IOException
	 **/
	@Test
	public void checkVhostOverride() throws IOException {
		...
		assertEquals("The virtual host is not correct", "my.vhost", vhost);
	}

	/**
	 * Tests eliminating the virtual host configuration - expects no vhost for the
	 * connection.
	 * 
	 * @throws IOException
	 **/
	...
	public void checkNoVhost() throws IOException {
		...
		assertEquals("The virtual host is present", null, vhost);
	}
	...
	private void transfer(int count, int window) throws IOException {
		...
		BaseHandler snk = Mockito.spy(BaseHandler.class);
		...
		Mockito.doAnswer((stubInvo) -> {
			...
			return null;
		}).when(snk).onDelivery(Mockito.any());
		sh.add(snk);

		BaseHandler src = ReactorTest.mockBaseHandler1(count);
		reactor.connectionToHost("127.0.0.1", ((AcceptorImpl) acceptor).getPortNumber(), src);
		reactor.run();
		reactor.free();
	...
	public void schedule() throws IOException {
		...
		reactor.free();
		reactorHandler.assertEvents(Type.REACTOR_INIT, Type.SELECTABLE_INIT, Type.SELECTABLE_UPDATED,
				Type.REACTOR_QUIESCED, Type.SELECTABLE_UPDATED, Type.SELECTABLE_FINAL, Type.REACTOR_FINAL);
	...
	private void assertReactorRunBarfsOnHandler(Reactor reactor, BarfException expectedException,
			Handler expectedHandler) {
		try {
			reactor.run();
			throw new AssertionFailedError("Reactor.run() should have thrown an exception");
		} catch (HandlerException handlerException) {
			assertSame("Linked exception does not match expected exception", expectedException,
					handlerException.getCause());
			assertSame("Handler in exception does not match expected handler", expectedHandler,
	...
	public void connectionRefused() throws IOException {
		...
		reactor.free();
		serverSocket.close();
		connectionHandler.assertEvents(Type.CONNECTION_INIT, Type.CONNECTION_LOCAL_OPEN, Type.CONNECTION_BOUND,
				Type.TRANSPORT_ERROR, Type.TRANSPORT_TAIL_CLOSED, Type.TRANSPORT_HEAD_CLOSED, Type.TRANSPORT_CLOSED,
				Type.CONNECTION_UNBOUND, Type.TRANSPORT);
	}
}
