...
public class EventExtensibilityTest extends TestCase {
	...
	/**
	 * Sample additional information that gets generated and passed around with the
	 * extension events. The information is stored inside
	...
	public interface ExtraEvent extends Event {
		...
		/**
		 * typesafe access to event type generated by this extension useful for handling
	...
	/**
	 * New handler methods for handling the extended event types. These methods can
	 * take {@link ExtraEvent} as a parameter to get typesafe access to
	 * {@link ExtraInfo}
	 *
	...
	/**
	 * Implementation of the default base class for ExtraHandler. All events should
	 * be forwarded to the {@link Handler#onUnhandled(Event)} method
	 */
	...
	public static class ExtraEventImpl implements ExtraEvent {
		 ...
		 */
		public static final ExtendableAccessor<Event, ExtraInfo> extraInfoAccessor = new ExtendableAccessor<>(
				ExtraInfo.class);
	...
	public void testExtendedType() throws IOException {
		...
		BaseHandler unaware = Mockito.spy(BaseHandler.class);
		boolean[] unawareSeenFoo = new boolean[] { false };
		Mockito.doAnswer((stubInvo) -> {
			Event e = stubInvo.getArgument(0);
			if (e.getEventType() == ExtraTypes.FOO) {
				unawareSeenFoo[0] = true;
			}
			return null;
		}).when(unaware).onUnhandled(Mockito.any());
		gen.add(empty);
		empty.add(extra);
		extra.add(unaware);
